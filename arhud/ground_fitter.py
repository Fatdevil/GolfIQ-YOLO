...
class GroundFitter:
    ...
    def update(
        ...
        centroid = tuple(
            sum(p[i] for p in plane_points) / len(plane_points) for i in range(3)
        )
        normal = (0.0, 1.0, 0.0)
        if self._ema_normal is None:
            ...
            self._ema_normal = tuple(
                self._ema_normal[i] * (1 - self._smoothing)
                + normal[i] * self._smoothing
                for i in range(3)
            )
        variance = 0.01 if len(plane_points) > 3 else 0.05
        confidence = compute_anchor_confidence(
            variance, tracking_quality, elapsed_since_reset
        )
        anchor = Anchor(
            position=centroid, normal=self._ema_normal, confidence=confidence
        )
