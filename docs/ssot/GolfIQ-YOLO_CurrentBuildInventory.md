...
- **GolfIQ Play mobile home** (`apps/mobile/src/screens/HomeScreen.tsx`): Greets the player, shows the current access plan, and links to play/range/trips placeholders. Uses `/api/profile/player`, `/api/access/plan`, and `/api/analytics/player` (Pro) to populate the view.
