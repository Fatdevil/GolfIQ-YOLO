...
- **Pro**: Coach share v2 – Pro-användare kan skapa delbara coachrapporter via `/api/coach/share/{run_id}` som bygger på `CoachRoundSummary` och delas med tokeniserade länkar. Webben läser `/coach/share/:sid` utan inloggning för ett mobilvänligt läsläge.【F:server/api/routers/coach.py†L1-L53】【F:web/src/pages/coach/CoachShareReportPage.tsx†L1-L172】【F:web/src/coach/ShareWithCoachButton.tsx†L1-L55】
...
- **Pro**: Player Profile v2 – kombinerar PlayerAnalytics, CoachDiagnosis och sekvens- + caddie-signaler till en AI-spelarmodell och 4-veckors utvecklingsplan via `/api/profile/player`, renderad på MyGolfIQ samt i coach share-sammanfattningen.【F:server/services/player_profile.py†L1-L226】【F:server/api/routers/profile.py†L1-L33】【F:web/src/pages/profile/MyGolfIQPage.tsx†L1-L130】【F:web/src/pages/coach/CoachShareReportPage.tsx†L120-L190】
- **Demo mode & onboarding**: `/api/demo/profile` levererar syntetisk demo-spelare (profil, analytics, diagnosis) som startar en första-gångs onboarding i MyGolfIQ och låter användaren växla till demo-läge utan att röra sina egna data.【F:server/services/demo_profile.py†L1-L146】【F:server/api/routers/demo.py†L1-L14】【F:web/src/pages/profile/MyGolfIQPage.tsx†L8-L210】
...
- **Coach-delning**: `/coach/share/:sid` visar publika coachrapporter baserade på delningstoken. Länkar skapas via `/api/coach/share/{run_id}` och payload hämtas från `/api/share/{sid}` utan ytterligare autentisering.【F:web/src/App.tsx†L145-L148】【F:server/api/routers/coach.py†L39-L53】【F:server/api/routers/share.py†L83-L117】
