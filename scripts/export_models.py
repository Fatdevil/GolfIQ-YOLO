...
def export_onnx(dummy_shape=(1,3,320,320), iters=25, lines=None):
    ...
    meta = {
        ...
        "opset":12, "input_shape":dummy_shape,
        "avg_latency_ms": round(avg_ms,2),
        "file": onnx_path, "size_mb": round(size_mb,2),
        "exporter": used
    }
    if lines is not None:
        log_and_capture(lines, f"Target: ONNX | opset=12 | input={dummy_shape} | exporter={used} | file={onnx_path} ({size_mb:.2f} MB) | avg_latency={avg_ms:.2f} ms")
    return meta
