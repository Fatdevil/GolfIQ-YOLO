...
def export_to_coreml(cfg: ExportConfig, onnx_path: Path) -> Path:
    ...
    output_path = cfg.output_dir / f"{cfg.model_name}.mlpackage"
...
def build_manifest(
    ...
    entries.append({
        ...
        "bytes": coreml_path.stat().st_size,
        "sha256": _hash_file(coreml_path),
    })

    entries.append({
        "format": "ncnn-param",
        "path": str(ncnn_paths["param"].relative_to(cfg.output_dir)),
        "bytes": ncnn_paths["param"].stat().st_size,
        "sha256": _hash_file(ncnn_paths["param"]),
    })
    entries.append({
        "format": "ncnn-bin",
        "path": str(ncnn_paths["bin"].relative_to(cfg.output_dir)),
        "bytes": ncnn_paths["bin"].stat().st_size,
        "sha256": _hash_file(ncnn_paths["bin"]),
    })
