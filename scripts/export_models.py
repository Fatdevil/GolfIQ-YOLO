...
def export_onnx(dummy_shape=(1,3,320,320), iters=25, lines=None):
    try:
        ...
        if lines is not None: log_and_capture(lines, f"ONNX: unavailable: {e}")
    ...
    try:
        ...
        conv_w = helper.make_tensor("conv_W", TensorProto.FLOAT, Wt.shape, Wt.tobytes(), raw=True)
        conv_b = helper.make_tensor("conv_B", TensorProto.FLOAT, B.shape,  B.tobytes(),  raw=True)
        conv   = helper.make_node("Conv", ["images","conv_W","conv_B"], ["c1"], pads=[1,1,1,1])
        relu   = helper.make_node("Relu", ["c1"], ["r1"])
        gap    = helper.make_node("GlobalAveragePool", ["r1"], ["p1"])
        shape  = helper.make_tensor("shape1", TensorProto.INT64, [2], np.array([1,8],dtype=np.int64))
        reshape= helper.make_node("Reshape", ["p1","shape1"], ["flat"])
        W2 = np.random.randn(8,4).astype(np.float32)
        b2 = np.zeros((4,), dtype=np.float32)
        W2t= helper.make_tensor("W2", TensorProto.FLOAT, W2.shape, W2.tobytes(), raw=True)
        b2t= helper.make_tensor("b2", TensorProto.FLOAT, b2.shape, b2.tobytes(), raw=True)
        matmul=helper.make_node("MatMul", ["flat","W2"], ["m1"])
        add   =helper.make_node("Add", ["m1","b2"], ["logits"])
        graph = helper.make_graph(
            [conv,relu,gap,reshape,matmul,add], "tiny_graph",
            [X],
    ...
    # Warmup
    for _ in range(5): sess.run(None, {"images": x})
    t0 = time.perf_counter()
    for _ in range(iters): sess.run(None, {"images": x})
    t1 = time.perf_counter()
    avg_ms = (t1 - t0) / iters * 1000.0
    size_mb = os.path.getsize(onnx_path) / 1e6

    meta = {
        "target":"ONNX","status":"ok",
        "opset":ONNX_OPSET, "input_shape":dummy_shape,
        "avg_latency_ms": round(avg_ms,2),
        "file": onnx_path, "size_mb": round(size_mb,2),
        "exporter": used
    }
    if lines is not None:
        log_and_capture(lines, f"Target: ONNX | opset={ONNX_OPSET} | input={dummy_shape} | exporter={used} | file={onnx_path} ({size_mb:.2f} MB) | avg_latency={avg_ms:.2f} ms")
    return meta

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--targets", default="onnx", help="comma list: onnx,tflite,coreml,ncnn (only onnx implemented; others are stubbed)")
    ...
    for t in ("tflite","coreml","ncnn"):
        if t in targets:
            s = {"target":t,"status":"skipped","reason":"not implemented in CI dry-run"}
            results.append(s); log_and_capture(lines, f"Target: {t.upper()} | SKIPPED (dry-run)")
